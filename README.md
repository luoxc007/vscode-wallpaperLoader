...
该插件加载的壁纸是全vscode屏的，而不是某一个窗口。

因为实现的原理是修改了vscode内置文件。

##### 但是这也导致了一个无法修复的问题：如果不再想使用本壁纸加载器，请先在配置栏中禁用掉壁纸加载器，待壁纸不见了，再从插件库中删除此插件，否则壁纸会一直存在。非常抱歉，这个问题作者无法解决。

因为vscode官方并没有给在用户卸载掉插件时提供一个回调的机会。
